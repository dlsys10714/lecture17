...
def test_divide_scalar_forward():
    np.testing.assert_allclose(
        ndl.divide_scalar(ndl.Tensor([[1.7, 1.45]]), scalar=12).data,
...
def test_broadcast_to_forward():
    np.testing.assert_allclose(
        ndl.broadcast_to(ndl.Tensor([[1.85, 0.85, 0.6]]), shape=(3, 3, 3)).data,
        np.array(
            [
...
def test_negate_forward():
    np.testing.assert_allclose(
        ndl.negate(ndl.Tensor([[1.45, 0.55]])).data, np.array([[-1.45, -0.55]])
    )


def test_transpose_forward():
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[1.95]], [[2.7]], [[3.75]]]), axes=(1, 2)).data,
    ...
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[2.45]], [[3.5]], [[0.9]]]), axes=(0, 1)).data,
        np.array([[[2.45], [3.5], [0.9]]]),
    )
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[4.4, 2.05], [1.85, 2.25], [0.15, 1.4]])).data,
...
def submit_forward():
    ...
    mugrade.submit(
        ndl.broadcast_to(ndl.Tensor([[1.95, 3.85, -0.6]]), shape=(3, 3, 3)).data
    )
    mugrade.submit(
        ndl.reshape(
            ndl.Tensor(
    ...
    mugrade.submit(
        ndl.transpose(ndl.Tensor([[[3.45]], [[2.54]], [[1.91]]]), axes=(0, 1)).data
    )
    mugrade.submit(
        ndl.transpose(ndl.Tensor([[4.45, 2.15], [1.89, 1.21], [6.15, 2.42]])).data
    )
...
def submit_backward():
    ...
    mugrade.submit(
        backward_check(ndl.reshape, ndl.Tensor(np.random.randn(5, 4)), shape=(5, 4, 1))
    )
    mugrade.submit(
        backward_check(ndl.reshape, ndl.Tensor(np.random.randn(5, 4)), shape=(2, 2, 5))
    )
    mugrade.submit(backward_check(ndl.negate, ndl.Tensor(np.random.randn(1, 4, 2))))
    mugrade.submit(
        backward_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 2, 4)), axes=(0, 2))
    )
    mugrade.submit(
        backward_check(
    ...
    mugrade.submit(
        backward_check(ndl.broadcast_to, ndl.Tensor(np.random.randn()), shape=(1, 3, 6))
    )
    mugrade.submit(
        backward_check(
    ...
    mugrade.submit(
        backward_check(ndl.summation, ndl.Tensor(np.random.randn(3, 6)), axes=(1,))
    )
    mugrade.submit(
        backward_check(
            ndl.summation,
    ...
    mugrade.submit(
        backward_check(ndl.summation, ndl.Tensor(np.random.randn(7, 8)), axes=(0, 1))
    )
    mugrade.submit(
        backward_check(
...
def gradient_check(f, *args, **kwargs):
    ...
    error = sum(
        np.linalg.norm(computed_grads[i].numpy() - numerical_grads[i])
        for i in range(len(args))
    )
    assert error < 1e-6
...
def test_softmax_loss_ndl():
    ...
    np.testing.assert_allclose(
        ndl.log(ndl.Tensor([[4.0], [4.55]])).data,
